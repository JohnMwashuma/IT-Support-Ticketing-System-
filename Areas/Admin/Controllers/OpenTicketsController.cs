...
namespace ITSUPPORTTICKETSYSTEM.Areas.Admin.Controllers
{
    ...
    [SelectedTab("OpenTickets")]
    public class OpenTicketsController : Controller
    {
        private const int TicketsPerPage = 5; 
        //
        // GET: /Admin/OpenTickets/

        public ActionResult Index(string searchBy, string search, int page = 1)
        {
            var totalTicketCount = Database.Session.Query<Ticket>().Count(t => t.Status.Any(a => a.Name == "Open"));
            ...
            if (User.IsInRole("client"))
            {
                ...
                currentTicketPage = currentTicketPage.Where(t => t.Status.Any(a => a.Name == "Open"));
            ...
            return View(new OpenTicketsIndex
            {
