...
namespace ITSUPPORTTICKETSYSTEM.Areas.Admin.Controllers
{
    ...
    [SelectedTab("NewTickets")]
    public class NewTicketsController : Controller
    {
        private const int TicketsPerPage = 5; 
        //
        // GET: /Admin/NewTickets/

        public ActionResult Index(string searchBy, string search, int page = 1)
        {

             var totalTicketCount = Database.Session.Query<Ticket>().Count(t => t.Status.Any(a => a.Name == "New"));
            ...
            return View(new NewTicketsIndex()
            {
        ...
        public ActionResult New()
        {
            ...
            return View("Form", new TicketsForm
            {
                ...
                Priorities = Database.Session.Query<Priority>().Select(priority => new PriorityCheckbox()
                {
                    Id = priority.Id,
                    IsChecked = false,
                    Name = priority.Name
        ...
        public ActionResult Edit(int id)
        {
            ...
            return View("Form", new TicketsForm
            {
               ...
               Priorities = Database.Session.Query<Priority>().Select(p => new PriorityCheckbox()
               {
                   Id = p.Id,
                   IsChecked = ticket.Priorities.Contains(p),
                   Name = p.Name
               }).ToList(),
                Comments = Database.Session.Query<Comments>().Select(comment => new CommentCheckbox()
                {
