...
namespace ITSUPPORTTICKETSYSTEM.Areas.Admin.Controllers
{
    ...
    public class PendingTicketsController : Controller
    {
        private const int TicketsPerPage = 5; 
        //
        // GET: /Admin/PendingTickets/

        public ActionResult Index(string searchBy, string search, int page = 1)
        {

            var totalTicketCount = Database.Session.Query<Ticket>().Count(t => t.Status.Any(a => a.Name == "Pending"));
            ...
            if (User.IsInRole("client"))
            {
                ...
                currentTicketPage = currentTicketPage.Where(t => t.Status.Any(a => a.Name == "Pending"));
            ...
            return View( new PendingTicketsIndex()
            {
